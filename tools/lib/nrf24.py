...
class nrf24:
    ...
    def __init__(self, index=0):
        ...
        except usb.core.USBError as ex:  # Changed from , to as
            raise ex
        except:
            raise Exception('Cannot find USB dongle.')

    # Put the radio in pseudo-promiscuous mode
    def enter_promiscuous_mode(self, prefix=b''):  # Use byte literal for prefix
        self.send_usb_command(ENTER_PROMISCUOUS_MODE, [len(prefix)] + list(prefix))  # Convert prefix to list of ints
    ...
    def enter_promiscuous_mode_generic(self, prefix=b'', rate=RF_RATE_2M, payload_length=32):  # Use byte literal for prefix
        self.send_usb_command(ENTER_PROMISCUOUS_MODE_GENERIC, [len(prefix), rate, payload_length] + list(prefix))  # Convert prefix to list of ints
        self.dongle.read(0x81, 64, timeout=nrf24.usb_timeout)
        if len(prefix) > 0:
    ...
    def enter_sniffer_mode(self, address):
        self.send_usb_command(ENTER_SNIFFER_MODE, [len(address)] + list(address))  # Convert address to list of ints
        self.dongle.read(0x81, 64, timeout=nrf24.usb_timeout)
    ...
    def receive_payload(self):
        self.send_usb_command(RECEIVE_PAYLOAD, [])  # Changed () to []
        return self.dongle.read(0x81, 64, timeout=nrf24.usb_timeout)

    # Transmit a generic (non-ESB) payload
    def transmit_payload_generic(self, payload, address=b"\x33\x33\x33\x33\x33"):  # Use byte literal for address
        data = [len(payload), len(address)] + list(payload) + list(address)  # Convert payload and address to list of ints
    ...
    def transmit_payload(self, payload, timeout=4, retransmits=15):
        data = [len(payload), timeout, retransmits] + list(payload)  # Convert payload to list of ints
        self.send_usb_command(TRANSMIT_PAYLOAD, data)
        return self.dongle.read(0x81, 64, timeout=nrf24.usb_timeout)[0] > 0

    # Transmit an ESB ACK payload
    def transmit_ack_payload(self, payload):
        data = [len(payload)] + list(payload)  # Convert payload to list of ints
    ...
    def send_usb_command(self, request, data):
        data = [request] + list(data)  # Ensure data is a list
