...
prefix_address = bytes.fromhex(common.args.prefix.replace(':', ''))  # Use bytes.fromhex() in Python 3
if len(prefix_address) > 5:
    raise Exception('Invalid prefix address: {0}'.format(common.args.prefix))  # Use args.prefix instead of args.address
...
while True:
    ...
    if len(value) >= 5:
        ...
        # Log the packet - No changes needed in Python 3
